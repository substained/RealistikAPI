...
func UserGraph(md common.MethodData) common.CodeMessager {
    shouldRet, whereClause, param := whereClauseUser(md, "users") // get it from /v1/users/ still
    if shouldRet != nil {
        return *shouldRet
    }

    // Hellest query I've ever done. <- funny now
    query := `
SELECT
    graph.id, graph.user_id, graph.leaderboard_rank, graph.unix_time

FROM users
LEFT JOIN graph
ON users.id=graph.id
WHERE ` + whereClause + ` AND ` + md.User.OnlyUserPublic(true) + `
LIMIT 1
`
    // Fuck.
    ...
    switch {
    case err == sql.ErrNoRows:
        return common.SimpleResponse(404, "u broke that shit")
    case err != nil:
        md.Err(err)
        return Err500
    }

    for modeID, m := range [...]*modeData{&r.STD} {

        if i := leaderboardPosition(md.R, modesToReadable[modeID], r.ID); i != nil {
            m.GlobalLeaderboardRank = i
		}
		cont := common.SanitiseString(*d.Data)
		_, err := md.DB.Exec("UPDATE graph SET leaderboard_rank = ? WHERE id = ? LIMIT 1", leaderboardPosition, r.ID())
		if err != nil {
			md.Err(err)
		}
	}